...
def check_period_need1(recent_month):  # month refers to monat
...
def check_comparisondates(df_akt):    # find the date to compare against
...
def need1(df):
    ...
    recent_akt, last_akt = check_comparisondates(df['AKT_DAY'])
    recent_month = str(recent_akt)[0:4]+str(recent_akt)[5:7]
    m, m_plus1, m_plus2 = check_period_need1(recent_month)
    alerts_n1 = []
        ...
        for soldtoname in sorted(df['SoldTo_Name'][df['CLM_Code']==clm].unique()):
            for salesname in df['SalesName'][(df['CLM_Code']==clm) & (df["SoldTo_Name"]==soldtoname)].unique():
                for monat in [m, m_plus1, m_plus2]:
                    ...
                    if difference_umwteuro < -50000:
                        type_need = "Need 1"
                        description = "Decline in " + str(difference_umwteuro) + " Euro"

                    elif difference_umwtpcs_percentage > 10:
                        type_need = "Need 1"
                        description = "Increase in " + str(difference_umwtpcs_percentage) + " %"
                    ...
                    if np.isnan(difference_umwteuro_percentage):
                        ...
                        cursor.execute("INSERT INTO dashboard_needonerecord(clm_code, soldtoname, salesname, monat, akt_day, last_umwteuro_amt, this_umwteuro_amt, diff_umwteuro, last_umwtpcs_amt, this_umwtpcs_amt, diff_umwtpcs, diff_umwtpcs_percent, sc_diff_umwteuro_percent, alert_type, alert_description) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (clm, soldtoname, salesname, monat, recent_akt, last_umwteuro, this_umwteuro, difference_umwteuro, last_umwtpcs, this_umwtpcs, difference_umwtpcs, difference_umwtpcs_percentage, difference_umwteuro_percentage, type_need, description))
...
def bp(df):
    ...
    #df['SumUMWTEuro_CLM_SoldToName'] = df.groupby(['CLM_Code', 'SoldTo_Name','AKT_DAY'])['UM_WT_Euro'].transform('sum')
    ...
    for clm in sorted(df["CLM_Code"].unique()):
        for soldtoname in sorted(df['SoldTo_Name'][df['CLM_Code']==clm].unique()):
            BP_thisakt = df["SumUMWTEuro_CLM_SoldToName"][(df['CLM_Code']==clm) & (df['SoldTo_Name']==soldtoname) & (df["MONAT"].isin([int(recent_monat),int(recent_monat)+1,int(recent_monat)+2])) & (df['AKT_DAY']==recent_akt)].max()
            BP_compare_akt = df["SumUMWTEuro_CLM_SoldToName"][(df['CLM_Code']==clm) & (df['SoldTo_Name']==soldtoname)& (df["MONAT"].isin([int(recent_monat),int(recent_monat)+1,int(recent_monat)+2])) & (df['AKT_DAY']==last_akt)].max()
            #changes = change( BP_thisakt, BP_compare_akt )
            changes = round((( 1 + ((BP_thisakt - BP_compare_akt)/(float(BP_thisakt + BP_compare_akt)/2)) ) * 100),1)

            alert_type = "Need 1"
            monat = "{}, {}, {}".format(int(recent_monat),int(recent_monat)+1,int(recent_monat)+2)
            cursor.execute("INSERT INTO dashboard_businessperformance(clm_code, alert_type, soldtoname, bp, monat) VALUES (%s, %s, %s, %s, %s)", (clm, alert_type, soldtoname, changes, monat))
